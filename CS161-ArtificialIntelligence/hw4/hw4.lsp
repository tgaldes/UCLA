...
;create a list of form (c c+1 c+2 ... k) and return it
(defun at-least-one-color (n c k) (cond
                                   ((= c k) (list (node2var n c k)))
                                   (T (append (list (node2var n c k)) (at-least-one-color n (+ c 1) k))))
  )

;this function is a helper for at-most-one-color
;for input n c k d (d = depth)
;branches when depth equals zero
;returns a list of (c' or c+1')...(c' or k')
(defun create-color-disjunctions (n c k d) (cond
                                            ((and (= (+ c d) k);base case for final branch, return nil
                                                  (= 0 d)) nil)
                                            ((= (+ c d) k) (list (list (* -1 (node2var n c k)) (* -1 (node2var n k k)))));base case for each branch, return a single clause
;below this line is when we have to branch, don't add anything explicitly to return value
                                            ((= d 0) (append (create-color-disjunctions n c k (+ d 1)) (create-color-disjunctions n (+ 1 c) k d)))
                                            (T (append (list (list (* -1 (node2var n c k)) (* -1 (node2var n (+ c d) k)))) (create-color-disjunctions n c k (+ d 1))))));add current and go one deeper
...
;return result of at-least-one-color and (a' or b') for every combination of a and b
(defun at-most-one-color (n c k) (cond
                                  (T (create-color-disjunctions n c k 0)))
  )
...
;
(defun generate-node-clauses (n k) (cond
                                    (T (append (list (at-least-one-color n 1 k)) (at-most-one-color n 1 k))))
  )
;helper for generate-edge-clauses
;does all the work, input is n1 n2 k d (where d is depth of recursion)
(defun generate-edge-disjunctions (m n k d) (cond
                                            ((= k d) (list(list (* -1 (node2var m d k)) (* -1 (node2var n d k)))))
                                            (T (append (generate-edge-disjunctions m n k (+ d 1))
                                                       (list (list (* -1 (node2var m d k)) (* -1 (node2var n d k))))))))
; EXERCISE: Fill this function
; returns *a list of clauses* to ensure that
; "the nodes at both ends of edge e cannot have the same color from the set {1,2,...,k}."
;
(defun generate-edge-clauses (e k) (cond
                                    ((= (car e) (cadr e)) nil)
                                    (T (generate-edge-disjunctions (car e) (cadr e) k 1))))
